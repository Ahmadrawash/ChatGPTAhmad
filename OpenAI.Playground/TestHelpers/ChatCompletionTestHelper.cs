...
internal static class ChatCompletionTestHelper
{
    public static async Task RunSimpleChatCompletionTest(IOpenAIService sdk)
    {
        ...
        {
            //*** tech  file ***
            //Stream s = File.OpenRead(Directory.GetCurrentDirectory() + "\\files\\" +  "imp invention in tech.csv");
            //** sci file ***
            Stream s = File.OpenRead(Directory.GetCurrentDirectory() + "\\files\\" + "imp invention in science.csv");

            var fs = new StreamReader(s);
            //*** tech file "result.txt" ***
            //Stream outputS = File.OpenWrite(Directory.GetCurrentDirectory() +  "\\files\\result.txt");
            //*** sci file "sresult.txt" ***
            Stream outputS = File.OpenWrite(Directory.GetCurrentDirectory() + "\\files\\sresult.txt");
            var outs = new StreamWriter(outputS);
            outs.WriteLine("Invention, Inventor, Date, ChatGPT_Result");
            outs.Flush();
            ...
            while (!fs.EndOfStream)
            {
                ...
                //if (line.IndexOf("invention") != -1 && line.IndexOf("inventor") != -1)
                //    continue;
                if (found == false)
                {
                ...
                String q = "", corrAns = "", Dtime = "";
                if (line != null && line.Split(',').Length != -1)
                {

                    q = "what is the name of the inventor of " + line.Split(',')[0];
                    if (line.Split(',').Length >= 2)
                        corrAns = line.Split(',')[1];
                    ...
                    try
                    {
                        ...
                        if (completionResult.Successful)
                        {
                            ...
                            outs.WriteLine(q + "," + corrAns + "," + Dtime + "," +
                ...
                //put current thread to sleep for 5 second
                Console.WriteLine("sleeping: " + c);
                Thread.Sleep(10000);
                Console.WriteLine("resume: " + c);
                c++;
